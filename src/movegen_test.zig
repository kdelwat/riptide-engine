...
test "generateLegalMoves - JetChess 1" {
    var moves = ArrayList(u32).init(test_allocator);
    generateLegalMoves(&moves, &position.fromFEN("r3k2r/p1ppqpb1/bn2pnp1/3PN3/1p2P3/2N2Q1p/PPPBBPPP/R3K2R w KQkq - 0 1"));
...
test "generateLegalMoves - JetChess 2" {
    var moves = ArrayList(u32).init(test_allocator);
    generateLegalMoves(&moves, &position.fromFEN("8/2p5/3p4/KP5r/1R3p1k/8/4P1P1/8 w - - 0 1"));
...
test "generateLegalMoves - JetChess 3" {
    var moves = ArrayList(u32).init(test_allocator);
    generateLegalMoves(&moves, &position.fromFEN("n1rb4/1p3p1p/1p6/1R5K/8/p3p1PN/1PP1R3/N6k w - - 0 1"));
...
test "generateLegalMoves - JetChess 4" {
    var moves = ArrayList(u32).init(test_allocator);
    generateLegalMoves(&moves, &position.fromFEN("5RKb/4P1n1/2p4p/3p2p1/3B2Q1/5B2/r6k/4r3 w - - 0 1"));
...
test "generateLegalMoves - JetChess 5" {
    var moves = ArrayList(u32).init(test_allocator);
    generateLegalMoves(&moves, &position.fromFEN("7r/3B4/k7/8/6Qb/8/Kn6/6R1 w - - 0 1"));
...
test "generateLegalMoves - JetChess 6" {
    var moves = ArrayList(u32).init(test_allocator);
    generateLegalMoves(&moves, &position.fromFEN("b1N1rb2/3p4/r6p/2Pp1p1K/3Pk3/2PN1p2/2B2P2/8 w - - 0 1"));
...
test "generateLegalMoves - Jetchess 7" {
    var moves = ArrayList(u32).init(test_allocator);
    generateLegalMoves(&moves, &position.fromFEN("1kN2bb1/4r1r1/Q1P1p3/8/6n1/8/8/2B1K2B w - - 0 1"));
...
test "generateLegalMoves - Jetchess 8" {
    var moves = ArrayList(u32).init(test_allocator);
    generateLegalMoves(&moves, &position.fromFEN("8/8/7K/6p1/NN5k/8/6PP/8 w - - 0 1"));
...
test "generateLegalMoves - Jetchess 9" {
    var moves = ArrayList(u32).init(test_allocator);
    generateLegalMoves(&moves, &position.fromFEN("8/2p5/2Pb4/2pp3R/1ppk1pR1/2n2P1p/1B2PPpP/K7 w - - 0 1"));
...
test "generateLegalMoves - Jetchess 10" {
    var moves = ArrayList(u32).init(test_allocator);
    generateLegalMoves(&moves, &position.fromFEN("8/6pp/8/p5p1/Pp6/1P3p2/pPK4P/krQ4R w - - 0 1"));
...
test "generateLegalMoves - Jetchess 11" {
    var moves = ArrayList(u32).init(test_allocator);
    generateLegalMoves(&moves, &position.fromFEN("rnbqkbnr/ppp1pppp/8/3p4/2P5/8/PP1PPPPP/RNBQKBNR w KQkq - 0 2"));
...
test "generateLegalMoves - Jetchess 12" {
    var moves = ArrayList(u32).init(test_allocator);
    generateLegalMoves(&moves, &position.fromFEN("rnbqkbnr/ppp1pppp/8/3p4/Q1P5/8/PP1PPPPP/RNB1KBNR b KQkq - 1 2"));
...
test "generateLegalMoves - Checkmate" {
    var moves = ArrayList(u32).init(test_allocator);
    generateLegalMoves(&moves, &position.fromFEN("rnb1kbnr/pppp1ppp/8/4p3/5PPq/8/PPPPP2P/RNBQKBNR w KQkq - 1 3"));
...
test "generateLegalMoves - Check from king" {
    var moves = ArrayList(u32).init(test_allocator);
    generateLegalMoves(&moves, &position.fromFEN("8/8/8/8/k7/2p5/2KP4/7R b - - 1 2"));
...
test "generateLegalMoves - Weird check" {
    var moves = ArrayList(u32).init(test_allocator);
    generateLegalMoves(&moves, &position.fromFEN("8/8/8/8/k7/8/2Kp4/2R5 b - - 1 3"));
    expect(countNonNullMoves(&moves) == 12);
    moves.deinit();
}

test "isKingInCheck" {
    expect(isKingInCheck(position.fromFEN("rnbqkbnr/pppppppp/8/8/8/8/PPPPPPPP/RNBQKBNR w KQkq - 0 1"), Color.white) == false);
    expect(isKingInCheck(position.fromFEN("rnbqkb1r/ppp1pp1p/6p1/1B1n4/3P4/2N5/PP2PPPP/R1BQK1NR b KQkq - 0 1"), Color.black) == true);
    expect(isKingInCheck(position.fromFEN("rnbq1b1r/pppkpppp/3pPn2/8/2PP4/8/PP3PPP/RNBQKBNR w KQkq - 0 1"), Color.white) == false);
    expect(isKingInCheck(position.fromFEN("rnbq1b1r/pppkpppp/3pPn2/8/2PP4/8/PP3PPP/RNBQKBNR b KQkq - 0 1"), Color.black) == true);
}