...
pub const p_position_startpos = m.map(UciCommand, toUnion("position_startpos", UciCommand), m.combine(.{ m.string("position startpos"), m.discard(m.opt(m.string(" moves "))), m.many(algebraic.long_algebraic_notation, .{ .collect = true }) }));
